// 当某月的天数
export function getDaysInOneMonth(date) {
...
export function getOtherMonth(date, str = "nextMonth") {
...
// 上个月末尾的一些日期
export function getLeftArr(date) {
  const arr = [];
...
// 下个月末尾的一些日期
export function getRightArr(date) {
  const arr = [];
...
// 获取某月的列表不包括上月和下月
export function getMonthListNoOther(date) {
  const arr = [];
...
// 默认是周一开始
export const sundayStart = false;

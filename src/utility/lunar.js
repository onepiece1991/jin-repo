 ...
 * 传入阳历月日得到阳历节日（中文）
 */
export function getSolarHoliday(m, d) {
  let txt;
  if (m == 1 && d == 1) {
    txt = "元旦";
  } else if (m == 5 && d == 1) {
 ...
 * 传入阴历月日得到阴历节日（中文）
 */
export function getLunarHoliday(m, d) {
  let txtL;
  if (m == 1 && d == 1) {
    txtL = "春节";
  } else if (m == 5 && d == 5) {
...
export function solar2lunar(y, m, d) {
  ...
  let isLunarH = false;
  if (
    getLunarHoliday(month, day) != "" &&
    getLunarHoliday(month, day) != undefined
  ) {
    isLunarH = true;
  }
  //传入的日期是否是阳历的节日
  let isSolarH = false;
  if (getSolarHoliday(m, d) != "" && getSolarHoliday(m, d) != undefined) {
    isSolarH = true;
  }
  ...
  return {
    ...
    astro: astro,
    isLunarH: isLunarH,
    lunarH: getLunarHoliday(month, day),
    isSolarH: isSolarH,
    solarH: getSolarHoliday(m, d),
  };
}
...
export function lunar2solar(y, m, d, isLeapMonth) {
  ...
  // leapOffset = 0;
  let leapMonthTxt;
  leapMonthTxt = leapMonth(y);
  // let leapDay = 0
  // leapDay = leapDays(y);
  if (isLeapMonth && leapMonthTxt != m) {
    return -1;
